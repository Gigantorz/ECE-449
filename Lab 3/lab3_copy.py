  ...
  def optimal(self, X):
    ...
    for filter in iter:
      ...
      for rate in learning_rate:
        ...
        model = self.make_model1(filter = temp_list, lr = rate)
    ...
    ideal_model = self.make_model1(filter = arr_optimal[0], lr = arr_optimal[1])
    ideal_model.summary()
    ideal_model.save("model1.keras")
    return ideal_model
...
if __name__ == "__main__":
    print("We are training for model 1 this shouldn't be to heavy!!!")
    data = pd.DataFrame()
    neuron = Classifier()
